...
function BoldText({ text, keyword }: { text: string; keyword: string }) {
  // split후 기준문자가 포함되려면 정규표현식 그룹() 사용(그냥 문자가 들어가면 배열에 포함되지 않음)
  // 정규표현식 내부에 변수사용시 RegExp 객체 사용
  // 'gi'는 옵션이며 g는 gloval, 문자열 전체 검색
  // i는 ignore case 대소문자 구분 안함
  ...
  return (
    <main className="p-[30px]">
      ...
      {!isPending && data?.result?.length === undefined && (
        <p className="text-gray-500 my-[40px] text-center font-">
          검색결과가 없습니다.
        </p>
      )}
      {data && data?.result?.length > 0 && (
