...
export default function NewsList({
  ...
  const { isPending, data, isError, error } = useQuery<{
    ...
    queryKey: ['newsList', page, paramsObj.keyword],
    queryFn: () => {
      return fetch(
        `http://localhost:9090/newsList?page=${page}&keyword=${paramsObj.keyword}`
      ).then((res) => res.json());
    },
  });
        ...
        <div className="flex items-start justify-between max-md:block">
          ...
          <div className="w-[964px] max-md:mt-[36px] max-md:w-full">
            ...
            >
              ...
              <button
                type="button"
                className="absolute w-[62px] h-[62px] bg-[url('/images/searchinputicon.svg')] bg-no-repeat bg-center right-0 top-0 max-sm:bg-[url('/images/headersearchicon.svg')] max-sm:h-[40px] max-sm:w-[54px]"
              ></button>
            </form>
